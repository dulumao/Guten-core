...
// func New(req *http.Request) *DefaultBinder {
...
func (b *DefaultBinder) Bind(i interface{}, c *rux.Context) (err error) {
	vars := c.Params
	params := map[string][]string{}

	for i, _ := range vars {
		params[i] = []string{vars[i]}
	}
	...
	if c.Req.ContentLength == 0 {
		if c.Req.Method == http.MethodGet || c.Req.Method == http.MethodDelete {
			if err = b.bindData(i, c.Req.URL.Query(), "query"); err != nil {
				return err
			}
			return
		}
	...
	case strings.HasPrefix(ctype, MIMEApplicationXML), strings.HasPrefix(ctype, MIMETextXML):
		if err = xml.NewDecoder(c.Req.Body).Decode(i); err != nil {
	...
	case strings.HasPrefix(ctype, MIMEApplicationForm), strings.HasPrefix(ctype, MIMEMultipartForm):
		...
		if err = b.bindData(i, c.Req.Form, "form"); err != nil {
			return err
		}
	default:
